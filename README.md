...
If you link FreeRTOS in your PlatformIO project, some `__weak` functions of the HAL are already defined there. The RTOS-less Cube project defines these functions by itself, leading to compilation errors when building the PlatformIO project. For this reason, you can specify the flag `--use-freertos`, which omits these functions from the copying process. **Be careful!** If this flag is set wrongly, your project will either fail at build time (because of double definitions) or the app will never start (because the functions are never defined at all).
